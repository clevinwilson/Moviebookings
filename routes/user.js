...
var authToken = "c54a325c3945db094cd6de7d55591e47"; // Your Auth Token 
...
router.get('/seat-layout/:screenId,:showId', verifyLogin, async (req, res) => {
  ...
  let theaterDetails =await userHelpers.gertheaterDetails(screenDetails.owner)
  ...
  userHelpers.getBookedSeats(req.params.showId).then((showDeatils) => {
    ...
    res.render('user/seat-layout', { screenDetails, "showDeatils": showDeatils,user: req.session.user,theaterDetails,latitude:userDetails.latitude,longitude:userDetails.longitude })
  })
...
router.get('/language/:type',(req,res)=>{
    userHelpers.movieLanguages(req.params.type).then((movies)=>{
      res.render('user/movie-language',{movies,user:req.session.user})
    })
})
